...
print(f"\nPTQ  → Acc: {ptq_acc*100:.2f}% | Latency: {ptq_latency:.2f} ms | Size: {ptq_size:5f} MB")
...
for epoch in range(num_qat_epochs):  # small retraining loop
   ...
   # After some warmup epochs, freeze observers and batchnorm statistics
    if epoch == freeze_epoch:
        # For newer PyTorch (torch>=1.13) use ao.quantization:
        try:
            import torch.ao.quantization as tq
        except ImportError:
            import torch.quantization as tq  # fallback for older versions

        qat_prep.apply(tq.disable_observer)
        for m in qat_prep.modules():
            if isinstance(m, nn.BatchNorm1d) or isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.BatchNorm3d):
                m.eval()

        print(f"==> Disabled observers and froze BN stats (if any) at epoch {epoch+1}")

    print(f"[QAT] epoch {epoch+1}/{num_qat_epochs}: loss={loss.item():.4f}")
...
# evaluate the model *before* conversion (still simulated quantization).
    pred_qat = qat_prep(X_test).argmax(dim=1)
...
print(f"\nQAT → Acc: {qat_acc*100:.2f}% | Latency: {qat_latency:.2f} ms | Size: {qat_size:.5f} MB")
...
print(f"\nGPTQ → Acc: {gptq_acc*100:.2f}% | Latency: {gptq_latency:.2f} ms | Size: {gptq_size:.5f} MB")
...
print("\n Conclusion:")
print(
    "In this experiment, QAT achieved slightly higher accuracy than both the FP32 baseline "
    "and PTQ, while keeping a similar model size and latency close to the original FP32 model.\n"
    "PTQ and the simulated GPTQ setup performed competitively and produced smaller model sizes, "
    "but in this run they did not surpass QAT in accuracy.\n"
)

print(
    "Because the fully converted int8 QAT model cannot run on the CPU backend available on Rorqual, "
    "the QAT accuracy reported here comes from the fake-quantized model (the model immediately before "
    "conversion). This still reflects how well the model adapted to quantization noise during training. "
    "PTQ accuracy, however, is measured using the fully converted int8 model.\n"
)

print(
    "All experiments were run on a Rorqual CPU node using PyTorch 2.6.0 and the fbgemm "
